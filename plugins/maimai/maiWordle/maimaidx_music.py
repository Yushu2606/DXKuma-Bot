...
# from .abstract_db_handle import abstract as sz
...
class MusicList(List[Music]):
    def by_version_for_plate(self,music_version) -> Optional[Music]:
        musiclist = []
        for music in self:
            if int(music.id) in [70,341,451,460,455,853,792,146,10146,189,419,687,731,11267,688,11213,712,11253]:
                continue
...
# 日服数据
# # obj = requests.get('https://download.fanyu.site/maimai/music_data.json').json()
# with open('./data/jp_music_data.json', 'r') as file:
#     jp_obj = json.load(file)

# jp_music_list: MusicList = MusicList(jp_obj)

# for __i in range(len(jp_music_list)):
#     jp_music_list[__i] = Music(jp_music_list[__i])
#     for __j in range(len(jp_music_list[__i].charts)):
#         jp_music_list[__i].charts[__j] = Chart(jp_music_list[__i].charts[__j])
